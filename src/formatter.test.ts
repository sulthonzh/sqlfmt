import { format, tokenize } from './formatter';
...
describe('tokenizer', () => {
  ...
  test('identifies keywords', () => {
    const tokens = tokenize('SELECT id, name FROM users WHERE active = TRUE');
  ...
  test('handles string literals', () => {
    const tokens = tokenize("WHERE name = 'John'");
  ...
  test('handles double-quoted identifiers', () => {
    const tokens = tokenize('SELECT "userId" FROM "UserTable"');
  ...
  test('handles block comments', () => {
    const tokens = tokenize('SELECT /* col */ 1');
    const comments = tokens.filter(t => t.type === 'comment');
    expect(comments).toHaveLength(1);
...
describe('formatter', () => {
  ...
  test('custom indent width', () => {
    const result = format('SELECT id, name FROM users WHERE active = 1', { indent: '    ' });
  ...
  test('comma before option', () => {
    ...
    const result = format(sql, { commaPosition: 'before' });
